...
export default function PracticeScreen() {
  ...
  useEffect(() => {
    ...
    try {      
      ...
      const endpoint = token ? API_ENDPOINTS.QUIZ_RANDOM : API_ENDPOINTS.QUIZ_DEMO;
      ...
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.resultsContainer}>
            ...
            {token && (
              <TouchableOpacity 
                style={[styles.newQuizButton, styles.historyButton]}
                onPress={() => Alert.alert('Coming Soon', 'Quiz history feature will be available in the next update!')}
              >
                <Text style={styles.newQuizButtonText}>View History</Text>
              </TouchableOpacity>
            )}
  ...
  const getVideoSource = (question) => {
    ...
    if (question.video && question.video.filePath) {
      ...
      videoKey = filePath.split('/').pop().split('.')[0].toLowerCase();
      console.log('Using video from API path:', question.video.filePath);
    ...
    const videoMapping = {
      ...
      'thank you': require('../../../assets/videos/thank.mp4'),
